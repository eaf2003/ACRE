...
Public Class TurnosFrm
    ...
    Public oCFG As New Datos.Configuracion
    ...
    Public Sub CargarListaSinBuscar(ByVal ListViewRes As ListView)
        ...
        'Dim vcantitems As Integer = ListView1.Items.Count

        'For i As Integer = 1 To vcantitems - 1
    ...
    Public Sub New()
        ...
        strletrasize = oCFG.DevolverValor("letrasize", "turnos.xml")
        strfoto = oCFG.DevolverValor("foto", "turnos.xml")
        strcolor = oCFG.DevolverValor("color", "turnos.xml")
        strbackcolor = oCFG.DevolverValor("backcolor", "turnos.xml")
        strcol1size = oCFG.DevolverValor("col1size", "turnos.xml")
        strcol2size = oCFG.DevolverValor("col2size", "turnos.xml")
        strcol3size = oCFG.DevolverValor("col3size", "turnos.xml")
        strcol4size = oCFG.DevolverValor("col4size", "turnos.xml")

       

        Dim colorlista As Color = Color.FromName(strcolor)
        Dim colorlistaback As Color = Color.FromName(strbackcolor)
        Dim LetraLista As New Font("Arial", CInt(strletrasize), FontStyle.Bold)

        imgLista = Image.FromFile(strfoto)
        ListView1.BackgroundImage = imgLista
        ListView1.BackColor = colorlistaback
        Me.BackColor = colorlistaback
        ListView1.ForeColor = colorlista
        ListView1.Font = LetraLista

        ListView1.View = View.Details
        ListView1.Columns.Add("ID", CInt(strcol1size))
        ListView1.Columns.Add("Nombre", CInt(strcol2size))
        ListView1.Columns.Add("Ingreso", CInt(strcol3size))
        ListView1.Columns.Add("Cantidad", CInt(strcol4size))
        ...
        oReservas = New BIZL.Reservas(FechaReservas, "ImprimioToken > 0 AND (ingreso is null OR ingreso = 0)", "CASE WHEN Ingreso IS NULL THEN 0 ELSE 1 END, Ingreso DESC, FechaImp")
        ...
        Timer1.Interval = 5000 '8seconds
        Timer1.Start()

    End Sub


    Private Sub ListView1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    Private Sub Refrescarbt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        RefrescarTodo()
        'TextBox1.Focus()
        'TextBox1.SelectAll()
